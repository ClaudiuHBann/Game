...
public:
	...
	inline Logger(const string& file, const bool logToBothStreams = false)
		: mStreamLogBoth(logToBothStreams) {
		mStream.open(file);
	}

	template<typename T>
	inline void Log(const T& data, const Type type = Type::NONE, const Color color = Color::NONE) {
		stringstream stream {};
		stream << mReset;
...
private:
	inline void Write(const stringstream& stream) {
		if (mStream.is_open()) {
			if (mStream) {
				mStream << stream.str();
			}
